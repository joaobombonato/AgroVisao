...
import { Fuel, Search, ChevronDown, Check, X, Gauge, Truck, Droplet, Plus, Minus, AlertTriangle, ChevronUp, DollarSign } from 'lucide-react';
      ...
      <div className="bg-white rounded-xl w-full max-w-sm shadow-2xl animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={enviar} className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                 <label className="block text-xs font-bold text-gray-700">Valor Unit. <span className="text-red-500">*</span></label>
                 <input 
                    type="number" 
                    placeholder="0,00" 
          ...
          <div className="border-t pt-2">
             <button type="button" onClick={() => setShowFrete(!showFrete)} className="text-xs font-bold text-gray-500 flex items-center gap-1 hover:text-green-600 mb-2">
             ...
             {showFrete && (
                 <div className="grid grid-cols-2 gap-3 bg-gray-50 p-2 rounded-lg animate-in slide-in-from-top-1">
                     <Input label="NF Frete" placeholder="Nº" value={form.nfFrete} onChange={(e:any) => setForm({ ...form, nfFrete: e.target.value })} />
                     <Input label="Valor Frete (R$)" type="number" placeholder="0,00" value={form.valorFrete} onChange={(e:any) => setForm({ ...form, valorFrete: e.target.value })} />
                 </div>
             )}
          </div>
          ...
          <button type="submit" className="w-full bg-green-600 text-white py-3 rounded-lg font-bold hover:bg-green-700">Confirmar Entrada</button>
        </form>
...
export default function AbastecimentoScreen() {
  ...
  const capacidadeTanque = estoqueInicial + totalComprado || 15000;
    ...
    <div className="space-y-4 p-4 pb-24">
      ...
      <div className={`rounded-xl p-4 text-white shadow-lg transition-colors ${nivelCritico ? 'bg-red-600 animate-pulse' : 'bg-gradient-to-r from-red-500 to-red-600'}`}>
         <div className="flex justify-between items-start mb-2">
             ...
             {nivelCritico && <AlertTriangle className="w-8 h-8 text-yellow-300 animate-bounce" />}
         </div>
         ...
         <p className="text-xs opacity-80 mt-1 text-right font-bold">{percentualTanque}% (Tanque 15k)</p>
      </div>
      ...
      <div className="bg-white rounded-lg border-2 p-4 shadow-sm">
         <h2 className="font-bold border-b pb-2 mb-3 text-gray-700 text-center uppercase text-sm">Controle de Estoque</h2>
         <button 
      ...
      <div className="bg-white rounded-lg border-2 p-4 shadow-sm">
        <h2 className="font-bold border-b pb-2 mb-3 text-gray-700 flex items-center gap-2">
            <Truck className="w-5 h-5 text-red-500"/> Registro de Saída
        </h2>
        
        <form onSubmit={enviar} className="space-y-4">
          
          <div className="space-y-1">
             <label className="block text-xs font-bold text-gray-700">Data <span className="text-red-500">*</span></label>
          ...
          <div>
            ...
            {showObs && (
                <div className="animate-in slide-in-from-top-1">
                    <textarea value={form.obs} onChange={(e) => setForm({...form, obs: e.target.value})} placeholder="Detalhes..." className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:border-red-500 focus:outline-none h-16 resize-none" />
                </div>
            )}
          </div>

          <div className="grid grid-cols-2 gap-4 bg-gray-50 p-3 rounded-xl border border-gray-200">
              <div className="col-span-2 text-xs font-bold text-gray-400 uppercase tracking-widest border-b pb-1 mb-1 text-center">Leitura da Bomba</div>
              
              <div className="space-y-1">
                 <label className="block text-xs font-bold text-gray-500 text-center">Inicial (Auto)</label>
                 <div className="bg-red-50 border border-red-100 rounded-lg p-2">
                     <input type="text" value={form.bombaInicial} readOnly className="w-full bg-transparent font-bold text-red-800 outline-none text-center" />
                 </div>
              </div>
              ...
              <div className="space-y-1">
                 ...
                 <input 
                    ...
                    className="w-full px-2 py-2 border-2 border-red-300 rounded-lg font-bold text-gray-900 focus:border-red-500 focus:outline-none text-center [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    placeholder="0"
                    required
                 />
              </div>

              <div className="col-span-2 text-xs font-bold text-gray-400 uppercase tracking-widest border-b pb-1 mb-1 mt-2 text-center">Hodômetro / Horímetro</div>
              ...
              <div className="space-y-1">
                 ...
                 <div className="bg-gray-200 border border-gray-300 rounded-lg p-2">
                     <input type="text" value={form.horimetroAnterior} readOnly className="w-full bg-transparent font-bold text-gray-500 outline-none text-center" placeholder="-" />
                 </div>
              </div>
          ...
          <div className="flex justify-between items-center bg-gray-800 text-white p-4 rounded-xl shadow-lg">
              ...
              <div className="text-center">
                   <p className="text-[10px] text-gray-400 uppercase font-bold">Média</p>
                   <p className="text-xl font-bold">{mediaConsumo}</p>
              </div>
              <div className="text-center">
                  <p className="text-[10px] text-gray-400 uppercase font-bold">Custo Est.</p>

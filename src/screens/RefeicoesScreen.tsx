    ...
    <div className="space-y-4 p-4 pb-24">
      ...
      <div className="bg-white rounded-lg border-2 p-4 shadow-sm">
        <h2 className="text-xs font-bold text-gray-500 mb-3 uppercase flex items-center gap-1">
          <Check className="w-3 h-3 text-orange-500"/> Últimas 5 Refeições
        </h2>
        {(() => {
          const recentes = [...(dados?.refeicoes || [])]
            .sort((a, b) => {
              const da = a.data_refeicao || a.data || '';
              const db = b.data_refeicao || b.data || '';
            ...
            .filter((v, i, arr) => {
               return arr.findIndex(t => 
                  t.id === v.id || 
                  (t.maquina === v.maquina && t.tipo_refeicao === v.tipo_refeicao && t.quantidade === v.quantidade && (t.data_refeicao || t.data) === (v.data_refeicao || v.data))
               ) === i;
            })
            .slice(0, 5);
              ...
              {recentes.map((r: any) => (
                <div key={r.id} className="text-xs flex justify-between items-center py-2 border-b last:border-0 border-gray-100">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-gray-400">{U.formatDate(r.data_refeicao || r.data)}</span>
                  </div>
                  <div className="flex flex-col items-end">
                    <span className="font-bold text-orange-600">{r.refeicao}</span>
                    <span className="text-[10px] text-gray-400">Total: R$ {U.formatValue(r.valor)}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="bg-white rounded-lg border-2 overflow-hidden shadow-sm">
        <div className="p-3 border-b bg-gray-50">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
                <Input type="date" value={filterData} onChange={(e: any) => setFilterData(e.target.value)} className="text-xs border rounded p-2 min-w-[140px]" />

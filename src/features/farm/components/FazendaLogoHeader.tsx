...
export function FazendaLogoHeader({ logoUrl, onFileChange, onAdjustClick, canEdit = true }: FazendaLogoHeaderProps) {
    return (
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center">
            <div className={`relative ${canEdit ? 'group cursor-pointer' : ''}`}>
            ...
            {canEdit && <p className="mt-4 text-[10px] text-gray-400 font-bold uppercase tracking-[0.15em] text-center">Toque para selecionar a logo</p>}
        </div>
    );
}
